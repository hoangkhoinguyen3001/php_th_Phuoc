...
<head>
    ...
    <title>Buổi 7 - Đăng kí tài khoản (Cách 3: PDO MySQL)</title>
</head>

<body>
    <?php require "connect.php"; ?>
    <div class="container">
        <h1 id="lblDangKi">Đăng kí tài khoản (Cách 3: PDO MySQL)</h1>
        <?php
        if (isset($_POST["tenTK"])) {
            ...
            $params = array("tenTK" => $tenTK);
            $result = ExecuteSelectQuery($sql, $params)[0]["SoLuong"];
            if ($result == 0) {
                // Thêm tài khoản vào CSDL
                $sql = "INSERT INTO TaiKhoan VALUES (:tenTK, :mk, :email, :sdt, :diaChi, :hoTen, 0, '', 1)";
                $params = array("tenTK" => $tenTK, "mk" => $mk, "email" => $email, "sdt" => $sdt, "diaChi" => $diaChi, "hoTen" => $hoTen);
